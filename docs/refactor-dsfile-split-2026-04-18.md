...
## ISSUE-003 — 5G/외부망 HLS remux 재생 실패 (DsHttp 줄바꿈 소실)
...
### 증상
5G 셀룰러에서 NAS 영상을 `NasHlsSource` 로 열면 VLC 가 재생을 시작하지 못하고
즉시 `[Player] 정지` 로 빠짐. 버퍼링 스피너만 계속 돌다가 다음 에피소드로
넘어가도 동일 현상. logcat:
```
libvlc demux: Could not parse playlist
libvlc demux: detected format: hls
libvlc demux: Could not open .../cache/hls_*.m3u8: Unknown error 1094995529
libvlc demux: garbage at input from 509, trying to resync...
```

### 경로 추적
처음엔 `Unknown error 1094995529` (= 0x41434345 = "ACCE") 를 VLC 의 file
access 모듈 실패로 오해하고 FileDescriptor / 로컬 HTTP 서버 대안을 검토함.
하지만 근거가 약해 진단 로그부터 심음:
- `body` 의 `\n` / `\r` 카운트
- rewrite 후 파일 exists / canRead / size
- m3u8 첫 500바이트 프리뷰
- VLC `-vv`

결과:
```
I/NAS: body 분석: length=243473 LF=0 CR=0
```
줄구분자 0개. 243473 바이트짜리 m3u8 이 한 줄로 뭉쳐 있음. 표준 위반이라
Synology 가 이런 걸 보낼 리 없음 → 클라이언트 측 문제.

### 원인
`DsHttp.httpGet` (그리고 `httpPost`, `httpPostForm`, `uploadFile` 응답 읽기)
가 모두 다음 패턴:
```java
...
```
`readLine()` 은 `\n`, `\r`, `\r\n` 을 모두 소비하고 반환값에 포함하지 않는다.
append 시 구분자가 통째로 사라짐. JSON 은 whitespace 무시라 지금껏 문제
없었지만, HLS m3u8 은 directive/URL 간 줄바꿈이 파싱 필수 요소. VLC 의
playlist demux 가 한 줄짜리 파일에서 `#EXTM3U`, `#EXTINF`, 세그먼트 URL 을
분리해내지 못해 `Could not parse playlist` → HLS 모듈도 fallback 실패.
...
```
4곳 (`httpGet`, `httpPostForm`, `httpPost`, `uploadFile` 응답) 을 이 헬퍼로
통일. 줄바꿈 보존. JSON 호출부는 무영향.

### 교훈
"확실한 솔루션 고르기 전에 진단 로그부터 심어라." VLC 의 `Unknown error
1094995529` 를 access 모듈 문제로 단정하고 FD / 로컬 HTTP 서버 같은 큰 변경을
꺼낼 뻔했음. 실제 근본 원인은 상위 레이어의 10줄짜리 readLine 루프. 한 곳
고치니 file:// access 는 정상 동작했고 대안은 전혀 필요 없었음.
