...
/**
 * XR 단말에서 Activity 를 Full Space 모드로 띄우기 위한 헬퍼.
 *
 * Galaxy XR(SM-I610)에서는 매니페스트 property 만으로는 Full Space 진입이 trigger 되지 않고,
 ...
 * (Spatial Film 앱의 실제 동작 메커니즘 — 결정적 4가지 조합 중 하나).
 *
...
public class XrFullSpaceLauncher {
    ...
    private void initSession(Activity activity) {
        if (!XrConfig.isXrDevice(activity.getPackageManager())) return;
        try {
